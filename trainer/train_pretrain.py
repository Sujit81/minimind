...
from trainer.trainer_utils import get_lr, Logger, is_main_process, lm_checkpoint, init_distributed_mode, setup_seed, init_model, SkipBatchSampler, setup_logger, compute_eta, load_checkpoint
...
def train_epoch(epoch, loader, iters, start_step=0, wandb=None, eval_loader=None):
    ...
    for step, (X, Y, loss_mask) in enumerate(loader, start=start_step + 1):
        ...
        Y = Y.to(args.device)
        loss_mask = loss_mask.to(args.device)
        lr = get_lr(epoch * iters + step, args.epochs * iters, args.learning_rate, args.warmup_ratio, args.min_lr_ratio)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--warmup_ratio", type=float, default=0.01, help="Warmup步数占比 (default: 1%)")
    parser.add_argument("--min_lr_ratio", type=float, default=0.1, help="最小学习率占比 (default: 10% of base LR)")
    ...
    # ========== 1.5 初始化日志 ==========
    setup_logger(log_dir=args.log_dir, log_name=f"pretrain_{args.hidden_size}{'_moe' if args.use_moe else ''}", level=args.log_level)
    
    # ========== 2. 配置目录、模型参数、检查ckp ==========
    os.makedirs(args.save_dir, exist_ok=True)
    lm_config = MiniMindConfig(hidden_size=args.hidden_size, num_hidden_layers=args.num_hidden_layers, use_moe=bool(args.use_moe), vocab_size=args.vocab_size)
