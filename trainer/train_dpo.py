...
def train_epoch(epoch, loader, iters, ref_model, lm_config, start_step=0, wandb=None, beta=0.1):
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        if step % args.log_interval == 0 or step == iters - 1:
            ...
            Logger(f'Epoch:[{epoch + 1}/{args.epochs}]({step}/{iters}) | loss: {current_loss:.4f} | dpo: {current_dpo_loss:.4f} | aux: {current_aux_loss:.4f} | lr: {current_lr:.2e} | elapsed: {eta["elapsed"]} | ETA: {eta["total_eta"]} | {eta["speed"]:.2f} steps/s')

            if wandb: wandb.log({"loss": current_loss, "dpo_loss": current_dpo_loss, "aux_loss": current_aux_loss, "learning_rate": current_lr, "speed": eta["speed"]})
...
if __name__ == "__main__":
    ...
    # ========== 1.5 初始化日志 ==========
    setup_logger(log_dir=args.log_dir, log_name=f"dpo_{args.hidden_size}{'_moe' if args.use_moe else ''}", level=args.log_level)
