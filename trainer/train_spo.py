...
def spo_train_epoch(epoch, loader, iters, ref_model, reward_model, reward_tokenizer, value_tracker, start_step=0, wandb=None):
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        if step % args.log_interval == 0 or step == iters:
            ...
            Logger(f'Epoch:[{epoch + 1}/{args.epochs}]({step}/{iters}) | policy_loss: {policy_loss_val:.4f} | aux: {current_aux_loss:.4f} | reward: {avg_reward_val:.4f} | baseline: {avg_baseline_val:.4f} | KL: {kl_val:.4f} | rho: {rho:.4f} | lr: {current_lr:.2e} | elapsed: {eta["elapsed"]} | ETA: {eta["total_eta"]}')
...
if __name__ == "__main__":
    ...
    # ========== 1.5 初始化日志 ==========
    setup_logger(log_dir=args.log_dir, log_name=f"spo_{args.hidden_size}{'_moe' if args.use_moe else ''}", level=args.log_level)
