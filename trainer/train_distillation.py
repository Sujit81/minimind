...
def train_epoch(epoch, loader, iters, teacher_model, lm_config_student, start_step=0, wandb=None, alpha=0.0, temperature=1.0):
    ...
    for step, (X, Y, loss_mask) in enumerate(loader, start=start_step + 1):
        ...
        if step % args.log_interval == 0 or step == iters - 1:
            ...
            current_distill_loss = distill_loss.item() if teacher_model is not None else 0.0
            current_lr = optimizer.param_groups[-1]['lr']
            eta = compute_eta(start_time, step, iters, epoch, args.epochs)

            Logger(f'Epoch:[{epoch + 1}/{args.epochs}]({step}/{iters}) | loss: {current_loss:.4f} | ce: {current_ce_loss:.4f} | distill: {current_distill_loss:.4f} | aux: {current_aux_loss:.4f} | lr: {current_lr:.2e} | elapsed: {eta["elapsed"]} | ETA: {eta["total_eta"]} | {eta["speed"]:.2f} steps/s')
...
if __name__ == "__main__":
    ...
    # ========== 1.5 初始化日志 ==========
    setup_logger(log_dir=args.log_dir, log_name=f"distill_s{args.student_hidden_size}_t{args.teacher_hidden_size}", level=args.log_level)
